...
export default class AuthProvider {
  ...
  constructor(sql: Sql, options: AuthProviderOptions) {
    ...
    this.#provider.onRefresh((user_id, token) =>
      this.on_refresh(user_id, token)
    );
  }
  ...
  async on_refresh(this: AuthProvider, user_id: string, token: AccessToken) {
    await this.sql`insert into access_tokens ${
      this.sql({
        user_id: user_id,
        access_token: await this.encrypt(user_id, token.accessToken),
        refresh_token: token.refreshToken &&
          await this.encrypt(user_id, token.refreshToken),
  ...
  private async derive_key(
    user_id: string,
    key_length: number = 256,
  ): Promise<CryptoKey> {
  ...
  private async encrypt(user_id: string, token: string): Promise<string> {
    ...
    const iv_and_cipher_text = new Uint8Array(
      iv.length + cipher_text.byteLength,
    );
  ...
  private async decrypt(
    ...
    const key = await this.derive_key(user_id);
    const iv_and_cipher_text = Uint8Array.from(
      atob(encrypted_token),
      (c) => c.charCodeAt(0),
    );
