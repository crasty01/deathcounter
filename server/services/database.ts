...
export default class Database {
  ...
  async initialize() {
    try {
      ...
      await this
        .#sql`create index if not exists idx_channel_id on deaths (channel_id);`;
      await this
        .#sql`create index if not exists idx_game_id on deaths (game_id);`;

      const channel_rows = await this.#sql<
        Array<ChannelTableRow>
      >`select * from channels`;
      const game_rows = await this.#sql<
        Array<GameTableRow>
      >`select * from games`;
      const deaths_rows = await this.#sql<
        Array<DeathTableRow>
      >`select * from deaths`;
  ...
  async createChannel(channel: ChannelTableRow) {
    ...
    const [database_channel] = await this.#sql<
      [ChannelTableRow]
    >`insert into channels ${this.#sql(channel)} returning *`;
    this.#channels.set(channel.channel_id, channel);
  ...
  async createGame(game: GameTableRow) {
    ...
    const [database_game] = await this.#sql<[GameTableRow]>`insert into games ${
      this.#sql(game)
    } returning *`;
    this.#games.set(game.game_id, game);
  ...
  async createDeath(death: DeathTableRow) {
    ...
    const [database_death] = await this.#sql<
      [DeathTableRow]
    >`insert into deaths ${this.#sql(death)} returning *`;
    if (!this.#deaths.has(death.channel_id)) {
      this.#deaths.set(death.channel_id, new Map());
    }
  ...
  async updateDeath(channel: ChannelTableRow, game: GameTableRow, n: number) {
    if (typeof n !== "number") {
      throw new Error("death increase is not a number");
    }

    const [database_death] = await this.#sql<
      [DeathTableRow]
    >`update deaths set deaths = deaths + ${n} where channel_id = ${channel.channel_id} and game_id = ${game.game_id} returning *`;
  ...
  async getChannelById(channel_id: string, omit_deaths = true) {
    let channel:
      | (typeof omit_deaths extends true ? ChannelTableRow : ChannelWithDeaths)
      | undefined;
    if (omit_deaths) {
      [channel] = await this.#sql<
        [ChannelWithDeaths?]
  ...
  async getChannelByName(channel_name: string, omit_deaths = true) {
    ...
    if (omit_deaths) {
      [channel] = await this.#sql<
        [ChannelWithDeaths?]
      >`select * from channels where channel_name = ${channel_name}`;
    } else {
      [channel] = await this.#sql<[ChannelWithDeaths?]>`select 
				channels.channel_id, 
				channels.channel_name, 
  ...
  async getGameById(game_id: string) {
    return (await this.#sql<
      [GameTableRow?]
    >`select * from games where game_id = ${game_id}`)?.[0];
  }
}
